 ...
 *   4. Cross-store flow                (PG parent fans out into Redis, collects back)
 *
...
import { Flow, Worker } from "effect-mq"
...
const program = Effect.gen(function*() {
  ...
  // 4 ─────────────────────────────────────────────────────────────────
  yield* scene(4, "Cross-store flow: Postgres parent, Redis children")
  const digest = yield* DigestFlow.execute({ tenant: "acme", audience: 12 })

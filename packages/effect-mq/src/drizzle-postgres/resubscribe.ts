 ...
 * `pollInterval`.
 *
 * The streak resets only when an attempt does not fail, never on how long an
 * attempt survived. With `@effect/sql-pg`'s `listen`, every failure that can
 * reach here is a *setup* failure: the stream acquires a connection and
 * issues `LISTEN` inside its setup effect, and once that returns nothing can
 * fail or end the queue — the listen client's upstream `error` handler is
 * empty, so an established subscription simply runs until its scope closes.
 * Uptime therefore says nothing about health: a thirty-second attempt is a
 * slow connect that failed, not a subscription that lived. Resetting on it
 * would hand a hung endpoint a full-speed retry and a fresh warning every
 * time, which is the exact flood this loop exists to stop.
 */
